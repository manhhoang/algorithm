package com.algo.data_structure.tree.segment_tree;
...
/**
 * The <tt>SegmentTree</tt> class is an structure for efficient search of
 ...
 * <p/>
 * Also it has been develop with <tt>LazyPropagation</tt> for range updates,
...
public class SegmentTree {
	...
	private int RSQ(int v, int from, int to) {
		...
		// If you did a range update that contained this node, you can infer the
		// Sum without going down the tree
	...
	private int RMinQ(int v, int from, int to) {
		...
		// If you did a range update that contained this node, you can infer the
		// Min value without going down the tree
		if (n.pendingVal != null && contains(n.from, n.to, from, to)) {
			return n.pendingVal;
		}
	...
	private void update(int v, int from, int to, int value) {
		...
		// The Node of the heap tree represents a range of the array with
		// bounds: [n.from, n.to]
		Node n = heap[v];
	...
	/**
	 * Read the following commands: init n v Initializes the array of size n
	 * with all v's set a b c... Initializes the array with [a, b, c ...] rsq a
	 * b Range Sum Query for the range [a, b] rmq a b Range Min Query for the
	 * range [a, b] up a b v Update the [a,b] portion of the array with value v.
	 * exit
	 * <p/>
	 * Example: <<init <<set 1 2 3 4 5 6 <<rsq 1 3 >>Sum from 1 to 3 = 6 <<rmq 1
	 * 3 >>Min from 1 to 3 = 1 <<input up 1 3 >>[3,2,3,4,5,6]
	 *
	 * @param args
	 */
