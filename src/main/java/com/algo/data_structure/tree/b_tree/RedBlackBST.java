 ...
 * class uses the convention that values cannot be <tt>null</tt>&mdash;setting the value associated
 * with a key to <tt>null</tt> is equivalent to deleting the key from the symbol table.
 * <p>
 * This implementation uses a left-leaning red-black BST. It requires that the key type implements
 * the <tt>Comparable</tt> interface and calls the <tt>compareTo()</tt> and method to compare two
 * keys. It does not call either <tt>equals()</tt> or <tt>hashCode()</tt>. The <em>put</em>,
 * <em>contains</em>, <em>remove</em>, <em>minimum</em>, <em>maximum</em>, <em>ceiling</em>, and
 * <em>floor</em> operations each take logarithmic time in the worst case, if the tree becomes
 * unbalanced. The <em>size</em>, and <em>is-empty</em> operations take constant time. Construction
 * takes constant time.
 * <p>
 * For additional documentation, see <a href="http://algs4.cs.princeton.edu/33balanced">Section
