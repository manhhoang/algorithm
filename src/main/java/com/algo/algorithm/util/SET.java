 ...
 * 
 * Set implementation using Java's TreeSet library. Does not allow duplicates.
 *
...
/**
 * The <tt>SET</tt> class represents an ordered set of comparable keys. It supports the usual
 ...
 * <p>
 * This implementation uses a balanced binary search tree. It requires that the key type implements
 * the <tt>Comparable</tt> interface and calls the <tt>compareTo()</tt> and method to compare two
 * keys. It does not call either <tt>equals()</tt> or <tt>hashCode()</tt>. The <em>add</em>,
 * <em>contains</em>, <em>delete</em>, <em>minimum</em>, <em>maximum</em>, <em>ceiling</em>, and
 * <em>floor</em> methods take logarithmic time in the worst case. The <em>size</em>, and
 * <em>is-empty</em> operations take constant time. Construction takes constant time.
 * <p>
 * For additional documentation, see <a href="http://introcs.cs.princeton.edu/44st">Section 4.4</a>
 * of <i>Introduction to Programming in Java: An Interdisciplinary Approach</i> by Robert Sedgewick
 * and Kevin Wayne.
 *
...
public class SET<Key extends Comparable<Key>> implements Iterable<Key> {
  ...
  /**
   * Returns all of the keys in the symbol table as an iterator. To iterate over all of the keys in
   * a set named <tt>set</tt>, use the foreach notation: <tt>for (Key key : set)</tt>.
   * 
  ...
  /**
   * Does this set equal <tt>y</tt>? Note that this method declares two empty sets to be equal even
   * if they are parameterized by different generic types. This is consistent with the behavior of
   * <tt>equals()</tt> within Java's Collections framework.
   * 
