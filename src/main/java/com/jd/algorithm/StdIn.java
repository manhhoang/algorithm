...
/**
 * The <tt>StdIn</tt> class provides static methods for reading strings and numbers from standard
 * input. See <a href="http://introcs.cs.princeton.edu/15inout">Section 1.5</a> of <i>Introduction
 * to Programming in Java: An Interdisciplinary Approach</i> by Robert Sedgewick and Kevin Wayne.
 * <p>
 * For uniformity across platforms, this class uses <tt>Locale.US</tt> for the locale and
 * <tt>"UTF-8"</tt> for the character-set encoding. The English language locale is consistent with
 * the formatting conventions for Java floating-point literals, command-line arguments (via
 * {@link Double#parseDouble(String)}) and standard output.
 * <p>
 * Like {@link Scanner}, reading a <em>token</em> also consumes preceding Java whitespace; reading a
 * line consumes the following end-of-line delimeter; reading a character consumes nothing extra.
 * <p>
 ...
 * <p>
 * Note that Java's UTF-8 encoding does not recognize the optional byte-order mask. If the input
 * begins with the optional byte-order mask, <tt>StdIn</tt> will have an extra character
 * <tt>uFEFF</tt> at the beginning. For details, see
...
public final class StdIn {
   ...
   * 
   * @return <tt>true</tt> if standard input is empty (except possibly for whitespae);
  ...
  /**
   * Returns true if standard input is empty (including whitespace). Use this to know whether the
